...
const AdminSettings = () => {
  ...
  const SettingItem = ({ title, path, clickable = true }) => {
    const baseClass = `
    ...
    <div className="p-7 pt-3 h-screen bg-white overflow-hidden">
      <div className="w-full md:w-3/4 lg:w-1/2 mx-auto flex flex-col h-full"> 
        ...
        <div className="space-y-3 flex-1">
          <SettingItem title="Change password" path="/admin-dashboard/settings/change-password" clickable={true} />
          <SettingItem title="Deactivate account" path="/admin-dashboard/settings/deactivate" clickable={true} />
          <SettingItem title="Delete account" path="/admin-dashboard/settings/delete" clickable={true} />
        </div>
      </div>
    </div>
  );
