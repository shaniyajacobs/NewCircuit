...
const AdminCoupons = () => {
  ...
  const fetchCoupons = async () => {
    try {
      ...
      const couponsList = await Promise.all(querySnapshot.docs.map(async doc => {
        const couponData = doc.data();
        // Fetch business name for each coupon
        const businessDoc = await getDocs(doc(db, 'businesses', couponData.businessId));
        const businessName = businessDoc.exists() ? businessDoc.data().businessName : 'Unknown Business';
        return {
          id: doc.id,
          ...couponData,
          businessName
