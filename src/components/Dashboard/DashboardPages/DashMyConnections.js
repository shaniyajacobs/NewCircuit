...
const DashMyConnections = forwardRef(({ onConnectionSelect, onConnectionDeselect }, ref) => {
  ...
  // Expose resetConnection method to parent component
  ...
  }, [location.state, connections, onConnectionSelect]);

  // Update parent component when connection selection changes
  useEffect(() => {
  ...
  }, [selectedConnection, onConnectionSelect, onConnectionDeselect]);
  ...
  return (
    <div className="p-7 bg-white rounded-3xl border border-gray-50 border-solid shadow-[0_4px_20px_rgba(238,238,238,0.502)] max-md:p-5 min-h-screen">
      {/* Sparks heading - only show when no connection is selected */}
