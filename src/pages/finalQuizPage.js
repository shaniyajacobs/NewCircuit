...
import { FooterShapes } from './Login'; // Import FooterShapes component
...
`;
    ...
    Array(shapesPerRow).fill().map((_, colIndex) => {
        ...
        return {
          size: Math.floor(seededRandom(shapeSeed) * (80 - 25)) + 25, // Reduce max size
...
/>
                  ...
                  <div className="bg-[#8F3F3F3] flex-grow flex flex-col items-center justify-center px-8 relative">
                      <div className="absolute top-16 w-3/4 h-4 bg-[#1E40AF] rounded-full"></div>
...
>
    Navigate to Dashboard
                  ...
                  {/* Use FooterShapesBlack for this page */}
                  <FooterShapesBlack />
              </div>
          );
      };
